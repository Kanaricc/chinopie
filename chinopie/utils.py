...
def set_fixed_seed(seed:Any):
    _logger.info("set fixed seed for random, torch, and numpy")
...
def set_train(model:nn.Module,eval_on_norm:bool=True):
    model.train()
    for mod in model.modules():
        ...
        if not has_require_grad:
            other_norms=(nn.LayerNorm,nn.GroupNorm)
            instance_norms=(nn.InstanceNorm1d,nn.InstanceNorm2d,nn.InstanceNorm3d,nn.LazyInstanceNorm1d,nn.LazyInstanceNorm2d,nn.LazyInstanceNorm3d)
            batch_norms=(nn.BatchNorm1d,nn.BatchNorm2d,nn.BatchNorm3d,nn.SyncBatchNorm,nn.LazyBatchNorm1d,nn.LazyBatchNorm2d,nn.LazyBatchNorm3d)
            if eval_on_norm and isinstance(mod,(*batch_norms,*instance_norms,*other_norms)):
                _logger.debug(f"eval frozen norm layer `{mod}`")
                mod.eval()
