...
class FileHelper:
    def __init__(self, disk_root: str, comment: str, ddp_session: Optional[DdpSession] = None):
...
class PhaseHelper:
    ...
    def __enter__(self):
        ...
        self._custom_probes=dict([(x,AverageMeter(x)) for x in self._custom_probe_name])

        self._loss_checked = False
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if not self._loss_checked:
            logger.error(f"no loss checked during phase {self._phase_name}")
...
class TrainHelper:
    ...
    def report_info(self):
        ...
        logger.warning(f"[PARAMS] dataset: train({len(self._data_train)}) val({len(self._data_val)}) test({len(self._data_test) if hasattr(self,'_data_test') else 'not set'})")
    ...
    def phase_test(self):
        ...
        return PhaseHelper(
            ...
            self._dataloader_test if hasattr(self,'_dataloader_test') else DataLoader(FakeEmptySet()),
            ddp_session=None,
            dry_run=self._dry_run,
